...
export async function loadState() {
  ...
  try {
    ...
    const canonicalBank = hasBankContent(bankFromTables)
      ? bankFromTables
      : hasBankContent(parsed.bank)
        ? normalizeBankForStorage(parsed.bank)
        : defaultState.bank;
    const safeState = {
      bank: canonicalBank,
    ...
    if (!hasBankContent(bankFromTables) && hasBankContent(canonicalBank)) {
      writeBankToTables(db, canonicalBank);
    }
...
export async function saveState(state) {
  ...
  const nextBank = state.bank !== undefined ? normalizeBankForStorage(state.bank) : current.bank;
  const safeState = {
    bank: nextBank,
